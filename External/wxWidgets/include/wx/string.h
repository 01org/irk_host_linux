...
#if wxGCS_SECURITY
    wxDECLARE_NO_ASSIGN_CLASS(wxCStrData);
#endif
};
...
#if wxGCS_SECURITY
      iterator& operator=(const iterator& i) { m_cur = i.m_cur; return *this; }
#endif
...
#if wxGCS_SECURITY
      const_iterator& operator=(const const_iterator& i) { m_cur = i.m_cur; return *this; }
#endif
...
#if wxGCS_SECURITY
      wxDECLARE_NO_COPY_TEMPLATE_CLASS(ConvertedBuffer, T);
#endif
  };
...
public:
        ...
        : m_str(str), m_buf(lenWanted)
    {
        ...
        if ( buf )
        {
            ...
            wxTmemcpy(m_buf.data(), (m_buf.length() + 1)*sizeof(CharType), buf, len + 1);
        }
