                 ...
                 const wxString& portName)
{
    ...
    else
    {
        ...
        wxStrcpy((wxChar*)lpDev + lpDev->wDriverOffset, driverName.length() + 1, driverName);

        lpDev->wDeviceOffset = (WORD)( lpDev->wDriverOffset +
                                       driverName.length() + 1 );
        wxStrcpy((wxChar*)lpDev + lpDev->wDeviceOffset, printerName.length() + 1, printerName);

        lpDev->wOutputOffset = (WORD)( lpDev->wDeviceOffset +
                                       printerName.length() + 1 );
        wxStrcpy((wxChar*)lpDev + lpDev->wOutputOffset, portName.length() + 1, portName);
