...
static inline wxChar* MYcopystring(const wxString& s)
{
    ...
    return wxStrcpy(copy, s.length() + 1, s.c_str());
}
...
static inline CharType* MYcopystring(const CharType* s)
{
    ...
    return wxStrcpy(copy, wxStrlen(s) + 1, s);
}
#endif
...
static CharType *wxDoRealPath (CharType *path)
{
  ...
  // wxGCS_SECURITY: We base our size on the size of the string knowing that the string currently contains a path and that
  //                 we will simply be shrinking it.
  size_t size = wxStrlen(path);
        ...
        if (*p == SEP)
          {
            if (p[1] == wxT('.') && p[2] == wxT('.') && (p[3] == SEP || p[3] == wxT('\0')))
              {
                ...
                if (q[0] == SEP && (q[1] != wxT('.') || q[2] != wxT('.') || q[3] != SEP)
                    && (q - 1 <= path || q[-1] != SEP))
                  {
                    size_t remaining = size - (q - path) / sizeof(CharType);
                    wxStrcpy (q, remaining, p + 3);
                    if (path[0] == wxT('\0'))
                      {
                        path[0] = SEP;
                        path[1] = wxT('\0');
            ...
            {
              size_t remaining = size - (p - path) / sizeof(CharType);
              wxStrcpy (p, remaining, p + 2);
            }
          }
      }
  }
                ...
                const wxString& user)
{
  ...
  wxStrcpy (dest, WXSIZEOF(dest), filename);
#ifdef __WINDOWS__
  ...
  if (!envname.empty() && !(val = wxGetenv (envname)).empty() &&
    ...
    {
        wxStrcpy (wxFileFunctionsBuffer, WXSIZEOF(wxFileFunctionsBuffer), tcp + val.length());
        *tcp++ = wxT('$');
        *tcp++ = wxT('{');
        size_t size = WXSIZEOF(dest) - (tcp - dest) / sizeof(wxChar);
        wxStrcpy (tcp, size, envname);
        wxStrcat (tcp, size, wxT("}"));
        wxStrcat (tcp, size, wxFileFunctionsBuffer);
    }
#endif
  ...
  {
    wxStrcpy(wxFileFunctionsBuffer, WXSIZEOF(wxFileFunctionsBuffer), wxT("~"));
    if (!user.empty())
           wxStrcat(wxFileFunctionsBuffer, WXSIZEOF(wxFileFunctionsBuffer), user);
    wxStrcat(wxFileFunctionsBuffer, WXSIZEOF(wxFileFunctionsBuffer), dest + len);
    wxStrcpy (dest, WXSIZEOF(dest), wxFileFunctionsBuffer);
  }
...
wxPathOnly (wxChar *path)
{
    if (path && *path)
    {
        ...
        // Local copy
        wxStrcpy (buf, WXSIZEOF(buf), path);
...
wxChar *wxGetTempFileName(const wxString& prefix, wxChar *buf, size_t numberOfElements)
{
    wxString filename;
    ...
    if ( buf && numberOfElements > 0 )
        wxStrcpy(buf, numberOfElements, filename);
    else
