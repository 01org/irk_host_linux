...
static bool wxGetHostNameInternal(wxChar *buf, int sz)
{
    ...
    *buf = wxT('\0');
    ...
    if ( ok )
    {
        wxStrlcpy(buf, sz, wxSafeConvertMB2WX(uts.nodename), sz);
    }
    ...
    if ( ok )
    {
        wxStrlcpy(buf, sz, wxSafeConvertMB2WX(cbuf), sz);
    }
...
bool wxGetFullHostName(wxChar *buf, int sz)
{
    ...
    if ( ok )
    {
        if ( !wxStrchr(buf, wxT('.')) )
        {
            ...
            {
                // the canonical name
                wxStrlcpy(buf, sz, wxSafeConvertMB2WX(host->h_name), sz);
            }
        }
...
bool wxGetUserId(wxChar *buf, int sz)
{
    ...
    *buf = wxT('\0');
    ...
    {
        wxStrlcpy (buf, sz, wxSafeConvertMB2WX(who->pw_name), sz);
        return true;
    }
...
bool wxGetUserName(wxChar *buf, int sz)
{
    ...
    *buf = wxT('\0');
    if ((who = getpwuid (getuid ())) != NULL)
    {
       ...
       wxStrlcpy(buf, sz, wxSafeConvertMB2WX(who->pw_gecos), sz);
       return true;
    }
